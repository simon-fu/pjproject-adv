...
function copy_libs () {
    ARCH=${1}
    ./copy-ice-lib.sh ${ARCH}
    return

    for SRC_DIR_0 in ${LIB_PATHS[*]}; do
        SRC_DIR="${PJSIP_DIR}/${SRC_DIR_0}"
#        DST_DIR="${SRC_DIR}-${ARCH}"
        DST_DIR="${PJSIP_DIR}/lib/${ARCH}/${SRC_DIR_0}"
        if [ -d "${DST_DIR}" ]; then
            rm -rf "${DST_DIR}"
        fi
        echo "SRC_DIR_0=$SRC_DIR_0"
